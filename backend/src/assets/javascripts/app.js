  ...
  methods: {
    ...
    createFeed: function (event) {
      ...
      var data = {
        ...
        use_proxy:
          form.querySelector("select[name=use_proxy]").value === "true",
      };
    ...
    compareOPML: function (event) {
      this.opmlCompareResult = [];
      var input = event.target;
      var form = document.querySelector("#opml-compare-form");
      this.$refs.menuDropdown.hide();
      api.compare_opml(form, true).then(function (result) {
        input.value = "";
        vm.opmlCompareResult = result;
        vm.settings = "compare-opml";
      });
    },
    logout: function () {
